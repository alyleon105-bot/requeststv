...
async function notifyStaff(request) {
  if (!PORTAL_CONFIG.notificationEndpoint) {
    throw new Error("Request email is not set up yet. Add your Google Apps Script Web App URL in config.js.");
  }

  await fetch(PORTAL_CONFIG.notificationEndpoint, {
    method: "POST",
    mode: "no-cors",
...
async function startSpeech() {
  ...
  speechStatus = "Starting speech input...";
  render();
  ...
  recognition.onerror = event => {
    listening = false;
    recognition = null;
    if (event?.error === "not-allowed" || event?.error === "service-not-allowed") microphoneReady = false;
...
async function checkExistingMicrophonePermission() {
  try {
    ...
    microphoneReady = permission.state === "granted";
    permission.onchange = () => {
      microphoneReady = permission.state === "granted";
    };
  } catch {}
